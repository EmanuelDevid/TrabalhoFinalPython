...
def nova_nota(text, tags):
    now = datetime.now()  # Data atual
    file = open(nome_do_arquivo, 'a')  # Abrir o arquivo
    id = str(sum(1 for line in open(nome_do_arquivo)) + 1)  # Gerar o id com base nas linhas
    texto = id + ";" + text + ";" + tags + ";" + now.strftime("%d/%m/%Y") + "\n"  # Adicionar o texto ao arquivo, os valores são separadas por ;
    file.writelines(texto)
    file.close()

def alterar_nota(nota_id, alterar_tag_ou_nota, novo):
    ...
    with open(nome_do_arquivo, 'w') as file:
        for i in texto:
            if texto.index(i) == nota_id - 1:
                ...
                if alterar_tag_ou_nota == "1":
                    file.write(id + ";" + novo + ";" + tags + ";" + data)
                elif alterar_tag_ou_nota == "2":
...
def listar_notas_filtro(tipo_filtro, filtro=""):
    file = open(nome_do_arquivo, 'r')
    list = []
    for line in file:
        id, text, tags, data = line.split(";")
...
def menu():
    while True:
        ...
        if opcao == "1":
            ...
            alterar_nota(id, id_ou_tag, novo)
            ...
            lista_retornada = []
            if sub_opcao == "1":
                lista_retornada = listar_notas()
            elif sub_opcao == "2":
                filtro = input("Digite o ID da nota desejada: ")
                lista_retornada = listar_notas_filtro("id", filtro)
            elif sub_opcao == "3":
                filtro = input("Digite o texto ou a tag da nota desejada: ")
                lista_retornada = listar_notas_filtro("nota_tags", filtro)
            elif sub_opcao == "4":
                filtro = input("Digite a data da criação da nota desejada: ")
                lista_retornada = listar_notas_filtro("create_date", filtro)
            else:
